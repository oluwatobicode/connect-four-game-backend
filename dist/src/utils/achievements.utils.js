 ...
 */
export async function checkMatchAchievements(gameId) {
    try {
        ...
        if (game.player2Id)
            humanPlayers.push(game.player2Id);
        for (const userId of humanPlayers) {
            ...
            const userElo = userId === game.player1Id
                ? game.player1.eloRating
                : game.player2?.eloRating;
            const opponentElo = userId === game.player1Id
                ? game.player2?.eloRating
                : game.player1.eloRating;
