...
export function getValidColumns(grid) {
    const validCols = [];
...
// ------------------------------------------------------------
export function evaluateWindow(window, playerVal) {
    let score = 0;
...
// ------------------------------------------------------------
export const scorePosition = (grid, playerVal) => {
    let score = 0;
...
export function minimax(board, depth, alpha, beta, isMaximizingPlayer) {
    ...
    if (isMaximizingPlayer) {
        ...
        for (const col of validLocations) {
            ...
            if (scorePosition(tempBoard, 2) >= 100)
                return { column: col, score: 1000000 };
            ...
            if (alpha >= beta)
                break; // Break out early to save computation time (Alpha-Beta Pruning)
        }
    ...
    // 4. Minimizing Player (The Human trying to give the AI a LOW score)
        ...
        for (const col of validLocations) {
            ...
            if (scorePosition(tempBoard, 1) >= 100)
                return { column: col, score: -1000000 };
...
// ------------------------------------------------------------
export function getBestMove(board, difficultyDepth = 4) {
