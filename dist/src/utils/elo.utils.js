 ...
 */
export const calculateNewElo = (ratingA, ratingB, result) => {
    ...
    const newRatingA = Math.max(ELO.MIN_RATING, Math.round(ratingA + ELO.K_FACTOR * (actualScoreA - expectedScoreA)));
    const newRatingB = Math.max(ELO.MIN_RATING, Math.round(ratingB + ELO.K_FACTOR * (actualScoreB - expectedScoreB)));
