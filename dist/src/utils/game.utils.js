...
export const createInitialBoard = () => {
    ...
    return Array.from({ length: GAME.BOARD_COLS }, () => Array(GAME.BOARD_ROWS).fill(0));
};
export const checkLowestRow = (grid, column) => {
...
export const checkWin = (grid, col, row, player) => {
    const playerVal = player === "player1" ? 1 : 2;
    const numCols = GAME.BOARD_COLS;
    const numRows = GAME.BOARD_ROWS;
    // 1. HORIZONTAL (-)
    let count = 1;
    for (let c = col + 1; c < numCols && grid[c][row] === playerVal; c++)
        count++;
    for (let c = col - 1; c >= 0 && grid[c][row] === playerVal; c--)
        count++;
    if (count >= 4)
        return true;
    // 2. VERTICAL (|)
    ...
    for (let r = row - 1; r >= 0 && grid[col][r] === playerVal; r--)
        count++;
    if (count >= 4)
        return true;
    // 3. DIAGONAL UP (/)
    count = 1;
    for (let i = 1; col + i < numCols &&
        row + i < numRows &&
        grid[col + i][row + i] === playerVal; i++)
        count++;
    for (let i = 1; col - i >= 0 && row - i >= 0 && grid[col - i][row - i] === playerVal; i++)
        count++;
    if (count >= 4)
        return true;
    // 4. DIAGONAL DOWN (\)
    count = 1;
    for (let i = 1; col + i < numCols && row - i >= 0 && grid[col + i][row - i] === playerVal; i++)
        count++;
    for (let i = 1; col - i >= 0 && row + i < numRows && grid[col - i][row + i] === playerVal; i++)
        count++;
    if (count >= 4)
        return true;
    return false;
};
export const isColumnFull = (grid, column) => {
