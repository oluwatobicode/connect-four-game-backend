...
export const checkAchievements = async (userId, gameResult) => {
    const newlyUnlocked = [];
    ...
    if (gameResult.winnerId === userId) {
        ...
        const consecutiveWins = recentGames.filter((g) => g.winnerId === userId).length;
        ...
        if (streak >= 3)
            await tryUnlock("win_streak_3");
        if (streak >= 5)
            await tryUnlock("win_streak_5");
    }
