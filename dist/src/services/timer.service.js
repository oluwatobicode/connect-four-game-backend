...
const getDisconnectTimerKey = (gameId, userId) => `${gameId}:${userId}`;
...
export const startTurnTimer = (gameId, playerToMoveId, io) => {
        ...
        try {
            ...
            if (game.currentTurn === playerToMoveId) {
                await prisma.game.update({
                    ...
                    data: {
                        status: "FORFEITED",
                        winnerId: game.player1Id === playerToMoveId
                            ? game.player2Id
                            : game.player1Id,
...
export const clearDisconnectTimer = (gameId, userId) => {
...
export const startDisconnectTimer = (gameId, droppingUserId, io) => {
    ...
    const timer = setTimeout(async () => {
        try {
            ...
            if (!game || game.status !== "IN_PROGRESS")
                return;
            const winnerId = game.player1Id === droppingUserId ? game.player2Id : game.player1Id;
            await prisma.game.update({
                where: { id: gameId },
                data: {
                    status: "ABANDONED",
