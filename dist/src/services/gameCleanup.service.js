...
export const cleanupStaleGames = async () => {
    try {
        const now = new Date();
        const unjoinedThreshold = new Date(now.getTime() - STALE_GAME_CLEANUP.UNJOINED_TIMEOUT);
        const inactiveThreshold = new Date(now.getTime() - STALE_GAME_CLEANUP.INACTIVE_TIMEOUT);
        ...
        if (totalDeleted > 0) {
            console.log(`[CleanupService] Cleanup complete. Deleted ${unjoinedResult.count} unjoined and ${inactiveResult.count} inactive games.`);
        }
        else {
